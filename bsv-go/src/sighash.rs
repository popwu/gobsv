...
// }
#[no_mangle]
pub extern "C" fn sighash_signature_new(
    signature: *mut BSVSignature,
    sighash_type: u8,
    sighash_buffer: &[u8],
) -> *mut BSVSighashSignature {
    let signature = unsafe { &mut *signature };
    let sighash_type = SigHash::try_from(sighash_type).unwrap(); // 现在 SigHash::from_u8 是一个存在的函数
...
// }
#[no_mangle]
pub extern "C" fn sighash_signature_to_hex(
    sighash_signature: *mut BSVSighashSignature,
) -> *mut libc::c_char {
    let sighash_signature = unsafe { &mut *sighash_signature };
    let hex = BSVSighashSignature::to_hex(&sighash_signature).unwrap();
...
// }
#[no_mangle]
pub extern "C" fn sighash_signature_to_bytes(
    sighash_signature: *mut BSVSighashSignature,
) -> ByteArray {
    let sighash_signature = unsafe { &mut *sighash_signature };
...
// }
#[no_mangle]
pub extern "C" fn sighash_signature_from_bytes(
    bytes: &[u8],
    sighash_buffer: &[u8],
) -> *mut BSVSighashSignature {
