...
// }
#[no_mangle]
pub extern "C" fn signature_recover_public_key(signature: *mut BSVSignature, message: *const u8, message_len: usize, hash_algo: SigningHash) -> *mut BSVPublicKey {
    let signature = unsafe { &mut *signature };
    let message = unsafe { std::slice::from_raw_parts(message, message_len) };
    let public_key = signature.recover_public_key(message, hash_algo.into()).unwrap();
...
// }
#[no_mangle]
pub extern "C" fn signature_recover_public_key_from_digest(signature: *mut BSVSignature, digest: *const u8, digest_len: usize) -> *mut BSVPublicKey {
    let signature = unsafe { &mut *signature };
...
// }
#[no_mangle]
pub extern "C" fn signature_to_compact_bytes(signature: *mut BSVSignature, recovery_info: Option<BSVRecoveryInfo>) -> ByteArray {
    let signature = unsafe { &mut *signature };
    let r = match recovery_info {
        Some(v) => Some(v.0),
        None => None,
...
// }
#[no_mangle]
pub extern "C" fn signature_to_compact_hex(signature: *mut BSVSignature, recovery_info: Option<BSVRecoveryInfo>) -> *mut libc::c_char {
    let signature = unsafe { &mut *signature };
    let r = match recovery_info {
        Some(v) => Some(v.0),
        None => None,
...
// }
#[no_mangle]
pub extern "C" fn signature_verify_message(signature: *mut BSVSignature, message: *const u8, message_len: usize, pub_key: *mut BSVPublicKey) -> bool {
    let signature = unsafe { &mut *signature };
    let message = unsafe { std::slice::from_raw_parts(message, message_len) };
