...
// }
#[no_mangle]
pub extern "C" fn bsm_is_valid_message(
    message: *const u8,
    message_len: usize,
    ...
    BSVBSM::verify_message(message, &signature, &address).is_ok()
}
...
// }
#[no_mangle]
pub extern "C" fn bsm_verify_message(
    message: *const u8,
    message_len: usize,
    signature: *mut BSVSignature,
    address: *mut BSVP2PKHAddress,
    ...
    let signature = unsafe { &mut *signature };
    let address = unsafe { &mut *address };
    BSVBSM::verify_message(message, &signature, &address).unwrap()
}
...
// }
#[no_mangle]
pub extern "C" fn bsm_sign_message(
    ...
    let message = unsafe { std::slice::from_raw_parts(message, message_len) };
    let signature = BSVBSM::sign_message(&priv_key, message).unwrap();
...
// }
#[no_mangle]
pub extern "C" fn bsm_sign_message_with_k(
    priv_key: *mut BSVPrivateKey,
    ...
    let message = unsafe { std::slice::from_raw_parts(message, message_len) };
    let signature = BSVBSM::sign_message_with_k(&priv_key, &ephemeral_key, message).unwrap();
    Box::into_raw(Box::new(signature))
}
